...
## Seperate total EGFs
data = pd.read_csv('{}_total_Boatwright.csv'.format(phase_distance),skipinitialspace=True)
...
ax[2,0].plot([1e17,1e23],[tao2fc(tao_lb,1e17),tao2fc(tao_lb,1e22)],linestyle='--',color='grey')
ax[2,0].plot([1e17,1e23],[tao2fc(tao_ub,1e17),tao2fc(tao_ub,1e22)],linestyle='--',color='grey')
...
# Corner frequency VS. Seismic moment w/ constant Stress drop lines
ax[2,1].plot([1e17,1e23],[tao2fc(tao_lb,1e17),tao2fc(tao_lb,1e22)],linestyle='--',color='grey')
ax[2,1].plot([1e17,1e23],[tao2fc(tao_ub,1e17),tao2fc(tao_ub,1e22)],linestyle='--',color='grey')
...
with open("fc_logtao_total__Boatwright{}.txt".format(phase_distance),'w') as f:
...
data = pd.read_csv('{}_certified_Boatwright.csv'.format(phase_distance),skipinitialspace=True)
data_array = np.array(data)
depth = []
fc = []
fc_std = []
...
# Corner frequency VS. Seismic moment w/ constant Stress drop lines
ax[2,2].plot([1e17,1e23],[tao2fc(tao_lb,1e17),tao2fc(tao_lb,1e22)],linestyle='--',color='grey')
ax[2,2].plot([1e17,1e23],[tao2fc(tao_ub,1e17),tao2fc(tao_ub,1e22)],linestyle='--',color='grey')
...
# Corner frequency VS. Seismic moment w/ constant Stress drop lines
ax[2,3].plot([1e17,1e23],[tao2fc(tao_lb,1e17),tao2fc(tao_lb,1e22)],linestyle='--',color='grey')
ax[2,3].plot([1e17,1e23],[tao2fc(tao_ub,1e17),tao2fc(tao_ub,1e22)],linestyle='--',color='grey')
...
with open("fc_logtao_certified_Boatwright_{}.txt".format(phase_distance),'w') as f:
    for i in np.arange(len(lat)):
        f.write("%s\t%s\t%s\t%s\n" % (fc[i],np.log10(tao[i]),lat[i],lon[i]))
f.close()
