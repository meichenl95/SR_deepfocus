...
data = pd.read_csv('pairsfile_{}_select_backup.csv'.format(phase_distance),skipinitialspace=True)
data_array = np.array(data)
depth = data_array[:,5]
fc = data_array[:,17]
fc_std = []
for i in np.arange(len(fc)):
    fc_std.append(np.float(data_array[i,18])*2)
M0 = data_array[:,9]
...
for i in np.arange(len(data_array[:,0])):
    if data_array[i,0] not in fc_pairs.keys() and data_array[i,15] > 3.0:
...
for j in np.arange(len(masterid)):
    print(masterid[j],tao[j],M0[j],lat[j],lon[j],depth[j],mag[j],fc[j],fc_lower_err[j],fc_upper_err[j],tao_lower_err[j],tao_upper_err[j],(7./16.*M0[i]/tao[j])**(1./3.)/1000)
