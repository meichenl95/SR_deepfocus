...
def stress_drop(fc,M0,PREM,depth):
    ...
    Vs = (radius-PREM[0][temp-1])/(PREM[0][temp]-PREM[0][temp-1])*(PREM[3][temp]-PREM[3][temp-1])+PREM[3][temp-1] # unit in m/s
...
ax[1,1].plot(depth,tao,linestyle='',marker='o',mfc='none',mec='black',markersize=4)
ax[1,1].set_yscale('log')
ax[1,1].set_xlabel('Depth(km)')
ax[1,1].set_ylabel('Stress drop(Pa)')

mean_depth = np.zeros(9)
flag = np.zeros(9)
mean_depth[2:8] = np.arange(6)*25+525
mean_depth_tao=np.zeros(len(mean_depth))
for i in np.arange(len(depth)):
    if depth[i]<425:
        mean_depth[0]=depth[i]
        mean_depth_tao[0] = np.log10(tao[i])
    if depth[i]>425 and depth[i]<475:
        mean_depth[1]=depth[i]
        mean_depth_tao[1]=np.log10(tao[i])
    ...
    if depth[i]>600 and depth[i]<650:
        mean_depth_tao[6] = (mean_depth_tao[6]*flag[6]+np.log10(tao[i]))/(flag[6]+1)
        flag[6] = flag[6]+1
    if depth[i]>625 and depth[i]<675:
        mean_depth_tao[7] = (mean_depth_tao[7]*flag[7]+np.log10(tao[i]))/(flag[7]+1)
        flag[7]=flag[7]+1
    if depth[i]>650 and depth[i]<700:
        mean_depth[8]=depth[i]
        mean_depth_tao[8]=np.log10(tao[i])
...
# Corner frequency VS. Seismic moment w/ constant Stress drop lines
ax[2,1].plot([1e18,1e22],[tao2fc(tao_lb,1e18),tao2fc(tao_lb,1e22)],linestyle='--',color='red')
ax[2,1].plot([1e18,1e22],[tao2fc(tao_ub,1e18),tao2fc(tao_ub,1e22)],linestyle='--',color='red')
ax[2,1].plot(mag2M0(np.array(mag)),fc,linestyle='',marker='s',markersize=6,mfc='yellow',mec='blue',markeredgewidth=1,alpha=0.6)
ax_twin = ax[2,1].twiny()
ax_twin.plot([M02mag(1e18),M02mag(1e22)],[tao2fc(tao_lb,1e18),tao2fc(tao_lb,1e22)],linestyle='--',color='none')
ax_twin.plot([M02mag(1e18),M02mag(1e22)],[tao2fc(tao_ub,1e18),tao2fc(tao_ub,1e22)],linestyle='--',color='none')
ax_twin.plot(mag,fc,linestyle='',marker='s',markersize=6,mfc='none',mec='none')
ax[2,1].text(1e20,0.025,'0.01 MPa',fontsize=10,color='red')
ax[2,1].text(1e21,0.3,'1000 MPa',fontsize=10,color='red')
ax[2,1].set_xscale('log')
ax[2,1].set_yscale('log')
ax[2,1].set_xlabel('Seismic moment')
ax[2,1].set_ylabel('Corner frequency(Hz)')
ax_twin.set_xlabel('Mw')
