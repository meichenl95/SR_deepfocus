...
for i in np.arange(len(data_array[:,0])):
    print(M0r2magdif(data_array[i,15]),data_array[i,10])
    if (data_array[i,0] not in fc_pairs.keys()) and np.abs(M0r2magdif(np.array(data_array[i,15]))-np.array(data_array[i,10]))<=0.5 and data_array[i,15] > 3.:
